...
public class PortalReconstructJob implements Runnable {
    ...
    public void run() {
        ...
        for (Integer[] frameCoord : portalBlocks) {
            world.getChunk(frameCoord[0] >> 4, frameCoord[2] >> 4).setBlockIdAt(frameCoord[0] & 15, frameCoord[1], frameCoord[2] & 15, frameCoord[3]);
            for (Player player : updatedPlayers) {
                player.getWorld().getPlayerManager().markBlockNeedsUpdate(frameCoord[0], frameCoord[1], frameCoord[2]);
            }
        }
    }
